...
class AllSignalementsPageState extends State<AllSignalementsPage> {
  ...
  Widget _buildSignalList() {
    return StreamBuilder(
      stream: _SignalementsService
          .getAllSignalements(), //_firebaseAuth.currentUser!.uid
      builder: (context, snapshot) {
        ...
        return FutureBuilder<List<Widget>>(
            future: Future.wait(snapshot.data!.docs
                .map((document) => _buildSignalItem(document))),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Center(
                    child:
                        Text('Error loading signalements ${snapshot.error}'));
              }
              if (!snapshot.hasData) {
      ...
      child: Column(
        ...
        children: [
          DetSignalement(
            ...
            signaleurName: signaleurName,
            signalantName: signalantName,
            leDate: date,
            aHeure: heure,
            signaleurJob: signaleurJob,
            signalantJob: signalantJob,
            raison: raison,
            signaleurUrl: signaleurUrl,
            signalantUrl: signalantUrl,
          ),
