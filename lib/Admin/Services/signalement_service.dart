...
class SignalementsService extends ChangeNotifier {
  ...
  Future<void> sendSignalement(
      String raison, String signlantId, String userId) async {
    CollectionReference signalements = _firestore.collection('Signalements');
  ...
  Future<void> deleteSignalement(String signalementID) async {
    try {
      ...
      if (signalementDoc.exists) {
        ...
        print(
            '!!!!!!!!!!!!! Signalement avec ID $signalementID supprimé avec succès.');
      } else {
        print(
            '!!!!!!!!!!!!!! Le signalement avec ID $signalementID n\'existe pas.');
      }
    } catch (e) {
      print(
          '!!!!!!!!!! Erreur lors de la suppression du signalement $signalementID: $e');
    }
  }
}
