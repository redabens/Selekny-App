...
class DemandeEnvoyeState extends State<DemandeEnvoye> {
  ...
  Future<void> _checkArtisansForLatestDemande() async {
    ...
    final demandeDoc = await demandecol
    ...
    try {
      ...
      print(
          "Erreur lors de la recherche de nom domaine : $e"); // Retourne une chaîne vide en cas d'erreur
    }
    final artisansRef =
        db.collection('users').where('role', isEqualTo: 'artisan');

    await artisansRef.get().then((QuerySnapshot artisansSnapshot) async {
      for (int i = 0; i < artisansSnapshot.docs.length; i++) {
        final artisanData =
            artisansSnapshot.docs[i].data() as Map<String, dynamic>;
        ...
        if (artisanDomaine == domainenom) {
          ...
          if (distance <= 30.0) {
            ...
            NotificationServices.sendPushNotification(
                token,
                "Offre d'un service $typeService",
                nomPrestation,
                "PublieDemande");
          }
        }
